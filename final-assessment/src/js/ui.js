...
function toggleLoading(isLoading) {
  ...
  if(isLoading){
    var load = document.querySelector('#search-butt');
    load.addEventListener('click', () =>{
      if(load.classList.contains('search-butt')){
        load.classList.remove('search-butt');
        load.classList.add('search-butt2');
      }
    ...
    var load = document.querySelector('#search-butt');
    if(load.classList.contains('search-butt2')){
      load.classList.remove('search-butt2');
      load.classList.add('search-butt');
    }
    load.innerText ="Search";
    load.disabled = false;
